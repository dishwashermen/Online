...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_array_key_exists('/ProjectId|UserId|StateIndex/', $_POST) && is_numeric($_POST['ProjectId']) && is_numeric($_POST['UserId'])) {
		...
		foreach ($_POST as $key => $val) if (! Preg_match('/ProjectId|UserId|DirectRule|AuthType|Autofill|StateIndex|Separate|SeparateIndex|HistoryState|Total|JournalIndex|JournalState|ProjectLimiting|Limit|ViewContent/', $key)) {
		...
		if (isset($_POST['Separate']) && $_POST['Separate'] == true) {
			...
			$NextSeparateIndex = $_POST['SeparateIndex'] + 1;
			
			$UserUpdate = $DBQ -> prep('UPDATE `scheme_users` SET `StateIndex` = :StateIndex, `SeparateIndex` = :NextSeparateIndex WHERE `Id` = :Id', array('Id' => $_POST['UserId'], 'NextSeparateIndex' => $NextSeparateIndex, 'StateIndex' => $_POST['StateIndex']));
			...
			echo json_encode(array('Action' => 'Separate', 'Ident' => 'Write', 'RuleData' => RuleData($_POST['StateIndex'], $_POST['UserId']), 'SeparateIndex' => (string) $NextSeparateIndex, 'Uid' => $_POST['UserId'], 'HistoryData' => $HistoryData, 'Reload' => true));
		...
		if (! isset($_POST['JournalState']) || $_POST['JournalState'] == 'END') {
			...
			if ($UserStatus == 3) require_once 'AfterEndProcessing.php';
			...
			if (isset($_POST['HistoryState'])) {
				...
				$UserUpdate = $DBQ -> prep('UPDATE `scheme_users` SET `StateIndex` = ' . $nextIndex . ', `SeparateIndex` = 1, `HistoryState` = :HistoryState' . ($UserStatus ? ', `Status` = ' . $UserStatus : '') . (count($LimitedHIT) ? ', `Limited` = "' . implode(',', $LimitedHIT) . '"' : '') . ' WHERE `Id` = :Id', array('Id' => $_POST['UserId'], 'HistoryState' => json_encode($HistoryState)));

			} else $UserUpdate = $DBQ -> prep('UPDATE `scheme_users` SET `StateIndex` = ' . $nextIndex . ($UserStatus ? ', `SeparateIndex` = 1, `Status` = ' . $UserStatus : '') . (count($LimitedHIT) ? ', `Limited` = "' . implode(',', $LimitedHIT) . '"' : '') . ' WHERE `Id` = :Id', array('Id' => $_POST['UserId']));
			...
			echo json_encode(array('Q' => GetQ($nextIndex), 'Action' => 'Resume', 'Ident' => 'Write', 'RuleData' => RuleData($nextIndex, $_POST['UserId']), 'StateIndex' => (string) $nextIndex, 'SeparateIndex' => $_POST['SeparateIndex'], 'HistoryState' => json_encode($HistoryState), 'HistoryData' => $HistoryData, 'Uid' => $_POST['UserId'], 'Reload' => true, 'LimitText' => ($Limited ? $LimitText : '')));
			...
			echo json_encode(array('Q' => GetQ($_POST['JournalState']), 'Action' => 'Resume', 'Ident' => 'Write', 'RuleData' => RuleData($_POST['JournalState'], $_POST['UserId']), 'StateIndex' => $_POST['JournalState'], 'SeparateIndex' => $_POST['SeparateIndex'], 'HistoryState' => json_encode($NewHistoryState), 'HistoryData' => $HistoryData, 'Uid' => $_POST['UserId'], 'Reload' => true, 'JournalIndex' => $NewJournalIndex));
